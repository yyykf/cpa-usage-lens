...
> Applies when upgrading an existing deployment **from v0.2.x or earlier to v0.3.0** (across the migration `20260605002633_add_api_key_dimension.sql`, the "API key dimension"). **A fresh install is unaffected** — just run all migrations before the first start.
...
## 13. Breaking migration: `request_events_hot` primary key single-column → composite (no zero-downtime)

> Applies when upgrading an existing deployment **from v0.3.x or earlier to v0.4.0** (across the migration `20260608155535_request_events_hot_composite_pk.sql`). **A fresh install is unaffected** — just run all migrations before the first start.

**Why it is breaking.** This migration changes the `request_events_hot` primary key from `(request_id)` to composite `(request_id, event_ts, total_tokens)`. The old collector binary inserts with `INSERT ... ON CONFLICT (request_id) DO NOTHING`. Postgres requires the `ON CONFLICT` inference columns to exactly match an existing unique constraint; once the primary key becomes composite, the old single-column `ON CONFLICT (request_id)` no longer matches any constraint and **every insert fails** (`there is no unique or exclusion constraint matching the ON CONFLICT specification`). So a running old binary against the migrated schema errors continuously — **rolling/zero-downtime upgrade is not supported here**.

**Required upgrade order (do not reorder).**

1. **Stop the collector** — set `COLLECTOR_ENABLED=false` on the production instance (or stop it). This halts queue consumption so nothing writes with the stale single-column `ON CONFLICT`.
   ```bash
   # in the production .env
   COLLECTOR_ENABLED=false
   docker compose -f docker-compose.prod.yml up -d
   ```
   > Brief data-loss caveat: while the collector is down longer than CPA's `redis-usage-queue-retention-seconds`, queued requests expire and are lost (the usual pop-on-read risk, see risk 2). Keep this window short.
2. **Run the migration** on the production database (CLI `supabase db push`, or paste `20260608155535_request_events_hot_composite_pk.sql` into the SQL Editor — fully idempotent and safe to re-run; existing rows migrate smoothly because the pre-migration `request_id` is unique, so the composite key is necessarily unique too — no backfill, no conflict).
3. **Deploy the new code** — bump `CUL_VERSION` to **v0.4.0** (or newer) and `up -d`. The new binary's `ON CONFLICT (request_id, event_ts, total_tokens)` matches the new primary key.
4. **Re-enable the collector** — set `COLLECTOR_ENABLED=true` and `up -d` to resume collection on the new schema + code.
...
> 仅在「升级既有部署、**从 v0.2.x 及更早升级到 v0.3.0**（跨过迁移 `20260605002633_add_api_key_dimension.sql`，API key 维度）」时适用。**全新部署不受影响**——首次启动前把所有迁移跑完即可。
...
## 十三、破坏性迁移：`request_events_hot` 主键 单列 → 复合（不支持零停机）

> 仅在「升级既有部署、**从 v0.3.x 及更早升级到 v0.4.0**（跨过迁移 `20260608155535_request_events_hot_composite_pk.sql`）」时适用。**全新部署不受影响**——首次启动前把所有迁移跑完即可。

**为什么是破坏性的。** 这条迁移把 `request_events_hot` 主键从 `(request_id)` 改为复合 `(request_id, event_ts, total_tokens)`。旧采集器 binary 的入库用 `INSERT ... ON CONFLICT (request_id) DO NOTHING`。Postgres 要求 `ON CONFLICT` 的推断列必须与某个现有唯一约束**完全匹配**；主键一旦变成复合，旧的单列 `ON CONFLICT (request_id)` 就匹配不到任何约束，**每次入库都会失败**（报 `there is no unique or exclusion constraint matching the ON CONFLICT specification`）。所以「旧 binary 跑在已迁移的 schema 上」会持续报错——**这里不支持滚动/零停机升级**。

**必须按序执行（不要调换顺序）。**

1. **停采集器**——把生产实例的 `COLLECTOR_ENABLED` 置为 `false`（或直接停掉）。这会停掉队列消费，确保没有任何写入再走旧的单列 `ON CONFLICT`。
   ```bash
   # 生产 .env 里
   COLLECTOR_ENABLED=false
   docker compose -f docker-compose.prod.yml up -d
   ```
   > 短暂丢数据提醒：采集器停机超过 CPA 的 `redis-usage-queue-retention-seconds` 期间，队列里的请求会过期丢失（即 pop 即删的固有风险，见风险 2）。务必把这个窗口压短。
2. **跑迁移**——在生产库执行（CLI `supabase db push`，或把 `20260608155535_request_events_hot_composite_pk.sql` 贴进 SQL Editor——全幂等、可安全重跑；迁移前 `request_id` 是唯一主键，故存量行的复合键必然唯一，平滑迁移、无需回填、不冲突）。
3. **部署新代码**——把 `CUL_VERSION` 升到 **v0.4.0**（或更新）并 `up -d`。新 binary 的 `ON CONFLICT (request_id, event_ts, total_tokens)` 与新主键匹配。
4. **重启采集器**——把 `COLLECTOR_ENABLED` 置回 `true` 并 `up -d`，在新 schema + 新代码上恢复采集。
